...
export default class Post { 
    ...
    save = async () => { 
        try {
            const res = await queryer.exec('INSERT INTO post (title, body, user_id) VALUES($1, $2, $3) RETURING *', ...this.values())
    ...
    static hydrate = rows => { 
       return rows.map(r => {
           return {id:r.id, title:r.title, body:r.body, modified:r.modified, created: r.created}
       })
    }
}